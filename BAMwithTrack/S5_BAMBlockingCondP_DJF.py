...
BAMMonth = BAMDate['date'].dt.month
...
for i in np.arange(n_BAM):
    ...
    if BAMday['date'].month not in [12, 1, 2]:
        print(f"Skip BAMloc {BAMloc} because out of DJF")
        continue
    DJFnBAMday += 1
...
B_B_day = DJFnBAMday*(2*T+1)
B_B_num = B_B.sum(axis=0)
B_LB_num = B_LB.sum(axis=0)
B_B_freq = (B_B_num/B_B_day)
B_LB_freq = (B_LB_num/B_B_day)
print('checkpoint:', flush=True)
print('total B_B days in DJF:',B_B_day, flush=True)
print('total B_B num in DJF:',np.nanmax(B_B_num), flush=True)
print('total B_LB num in DJF:',np.nanmax(B_LB_num), flush=True)

# get the DJF climatology
DJF_mask = Date0['date'].dt.month.isin([12, 1, 2])
DJF_indices = np.where(DJF_mask)[0]
B_freq_DJF = B_freq_3d[DJF_indices, :, :]
B_freq_DJF_2d = np.nansum(B_freq_DJF, axis=0)  # average over DJF months
print(B_freq_DJF_2d.shape, flush=True)
print(np.nanmax(B_freq_DJF_2d), flush=True)
B_freq_clima = B_freq_DJF_2d/len(DJF_indices)  # climatology over DJF
print(np.nanmax(B_freq_clima), flush=True)
       
minlev =0
maxlev = B_B_freq[lat_mask,:].max()
levs = np.linspace(0.02,0.20,20)
...
plt.savefig("BlockingPeakingPredict_Fig3_DJF.png",dpi=600)
plt.close()
