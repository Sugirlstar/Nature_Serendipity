...
BI = BI_full_interp
BAMDate = Date0 # BAMDate: the Date index for BAM index, with February 29
...
BAM_event_DJF_BI = [bi for idx, bi in zip(BAM_event_all, BAM_event_BI_all) if idx in BAM_event_DJF]
print('BAM_event_DJF:', BAM_event_DJF, flush=True)
print('BAM_event_DJF_BI:', BAM_event_DJF_BI, flush=True)
...
print(trackPoints_array.shape, flush=True) # 0 or 1
trackPoints_2D = np.any(trackPoints_array, axis=1).astype(int)
nt = trackPoints_2D.shape[0]
# keep only the DJF dates
mask = np.ones(nt, dtype=bool)
mask[DJF_index_preceding] = False
trackPoints_2D[mask, :] = 0
...
print(trackPoints_2D.shape, flush=True)
print(blockingEidArr_2D.shape, flush=True)
print(B_B.shape, flush=True)
# transfer BI and blockingEidArr_2D to the same time dimension as trackPoints_2D
# BI is daily, blockingEidArr_2D is daily, trackPoints_2D is 6-hourly
# so we need to repeat BI and blockingEidArr_2D 4 times (6-hourly)
print('----shape check----', flush=True)
B_B_6hr = np.repeat(B_B, 4, axis=0)
B_LB_6hr = np.repeat(B_LB, 4, axis=0)
print(B_B_6hr.shape, flush=True)
blockingEidArr_6hr = np.repeat(blockingEidArr_2D, 4, axis=0)
print(blockingEidArr_6hr.shape, flush=True)
print(trackPoints_2D.shape, flush=True)

# check the probability of Blocking under HB or LB
...
nDJF = len(DJF_index) # number of DJF days
ntime = nDJF*4 # 6-hourly
...
plt.tight_layout()
plt.show()
plt.savefig(f'BAM2D_BlockingFreq_AlongLon_{rgname}_Type{typeid}_{cyc}_DJF.png', dpi=300)

# define track days within the study region:
# # 01 plot the track frequency along lon
# print('trackfreq:',T_freq_clima[0],T_freq_clima[-1],flush=True)
# fig, ax = plt.subplots(figsize=(10, 5))
# ax.plot(tracklon, T_freq_clima, color='black', label='AC tracks')
# # target region shading
# ax.axvspan(lon_min, lon_max, color='yellow', alpha=0.3, label=f'Region {lon_min}-{lon_max}')
# # labels
# ax.set_xlabel('Longitude')
# ax.set_ylabel('AC Track Frequency')
# ax.legend()
# ax.grid(True)
# plt.tight_layout()
# plt.show()
# plt.savefig(f'Track2D_AlongLon_{rgname}_Type{typeid}_{cyc}.png', dpi=300)

# %% get the tracking days: those days where the region with at least 1 track
...
# interpolate the trackPoints_2D to 360 lat grid points (from 512)
ntime, _ = trackPoints_2D.shape
lon_new = Blklon
trackPoints_2D_resampled = np.zeros((ntime, len(lon_new)))
...
kernel = np.ones((5, 11), dtype=int)  # (time_window, lon_window)
result = convolve(trackPoints_2D_resampled, kernel, mode='constant', cval=0)
...
print('trackBlk shape:', trackBlk.shape, flush=True)
trackBlk_num = np.nansum(trackBlk,axis=0)
track_B_freq = (trackBlk_num/trackdays) # frequency of blocking under track days


# get the probability under highBAM stage
highBAMindex = np.where(B_B_6hr[:,0]>0)[0]
print('highBAMindex:', highBAMindex, flush=True)
trackPoints_2D_mask_HB = trackPoints_2D_mask[highBAMindex,:]
blockingEidArr_6hr_HB = blockingEidArr_6hr[highBAMindex,:]  
print('trackPoints_2D_mask_HB shape:', trackPoints_2D_mask_HB.shape, flush=True)
print('blockingEidArr_6hr_HB shape:', blockingEidArr_6hr_HB, flush=True)
trackBlk_HB = blockingEidArr_6hr_HB * trackPoints_2D_mask_HB
trackBlk_HB_num = np.nansum(trackBlk_HB,axis=0)
trackdays_HB = np.nansum(trackPoints_2D_mask_HB, axis=0)
print('trackBlk_HB_num:', trackBlk_HB_num, flush=True)
print('trackdays_HB:', trackdays_HB, flush=True)
...
ax.plot(Blklon, B_freq_clima, color='black', label=f'Climatology, n={int(np.round(nDJF))} days')
# target region shading
...
plt.tight_layout()
plt.show()
plt.savefig(f'BAMwithACs_BlockingFreq_AlongLon_{rgname}_Type{typeid}_{cyc}_DJF.png', dpi=300)
