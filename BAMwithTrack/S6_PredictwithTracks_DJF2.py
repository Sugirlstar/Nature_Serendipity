...
# time for tracks
tracktime = pd.to_datetime(ds['time'].values)
DJF_mask = tracktime.month.isin([12, 1, 2])  
track_DJF_index = np.where(DJF_mask)[0] 
print('Track DJF_index:', track_DJF_index, flush=True)
print(len(track_DJF_index), flush=True)
offsets = np.arange(-leadD, 1)  
track_DJF_index_preceding = np.unique(np.concatenate([track_DJF_index + offset for offset in offsets]))
...
mask[track_DJF_index_preceding] = False
...
plt.tight_layout()
plt.show()
plt.savefig(f'BAMwithACs_BlockingFreq_AlongLon_{rgname}_Type{typeid}_{cyc}_{leadD}daylead_DJF.png', dpi=300)
