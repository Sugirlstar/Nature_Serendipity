...
# read in the fake blocking index
fake_blockingEidArr_6hr = np.load(f'/scratch/bell/hu1029/LGHW/fake_blockingEidArr_6hr_Type{typeid}_{rgname}_{ss}.npy')
fake_nonblockingintime_6hr = np.load(f'/scratch/bell/hu1029/LGHW/fake_innonBlockdays_blockingEidArr_6hr_Type{typeid}_{rgname}_{ss}.npy')
...
fake_blk_under_BAM = fake_blockingEidArr_6hr * B_B_6hr
fake_blk_under_LB = fake_blockingEidArr_6hr * B_LB_6hr
fake_B_B_num = np.nansum(fake_blk_under_BAM,axis=0)
fake_B_LB_num = np.nansum(fake_blk_under_LB,axis=0)
fake_B_B_freq = (fake_B_B_num/B_B_day)
fake_B_LB_freq = (fake_B_LB_num/B_B_day) # frequency of blocking under low BAM condition

fake_blknon_under_BAM = fake_nonblockingintime_6hr * B_B_6hr
fake_blknon_under_LB = fake_nonblockingintime_6hr * B_LB_6hr
fake_B_B_num_non = np.nansum(fake_blknon_under_BAM,axis=0)
fake_B_LB_num_non = np.nansum(fake_blknon_under_LB,axis=0)
fake_B_B_freq_non = (fake_B_B_num_non/B_B_day)
fake_B_LB_freq_non = (fake_B_LB_num_non/B_B_day) # frequency of blocking under low BAM condition    

#%% 04 plot high/low BAM and climatology
fig, ax = plt.subplots(figsize=(10, 5))
ax.plot(Blklon, B_B_freq, color='red', label='High BAM')
ax.plot(Blklon, fake_B_B_freq, color='red', linestyle='--', label='FakeBlk High BAM')
ax.plot(Blklon, fake_B_B_freq_non, color='red', label='FakeBlk Non-BlockingTime High BAM', linestyle=':')

ax.plot(Blklon, B_LB_freq, color='blue', label='Low BAM')
ax.plot(Blklon, fake_B_LB_freq, color='blue', linestyle='--', label='FakeBlk Low BAM')
ax.plot(Blklon, fake_B_LB_freq_non, color='blue', label='FakeBlk Non-BlockingTime Low BAM', linestyle=':')
...
plt.tight_layout()
plt.show()
plt.savefig(f'withFake2Senario_BAM2D_BlockingFreq_AlongLon_{rgname}_Type{typeid}_{cyc}.png', dpi=300)
...
# trackwithBlocking_fake
trackBlk_fake = fake_blockingEidArr_6hr * trackPoints_2D_mask
print('arr0/1 fake blocking with track preceding shape:', trackBlk_fake.shape, flush=True)
trackBlk_fake_num = np.nansum(trackBlk_fake,axis=0)
print('fake blocking with track preceding number at each lon:', trackBlk_fake_num, flush=True) # (360,), a value each day
track_B_freq_fake = (trackBlk_fake_num/trackdays) # frequency of fake blocking under track days
print('frequency of fake blocking under track days out of all track days:', track_B_freq_fake, flush=True)
# trackwithBlocking_fake_non
trackBlk_fake_non = fake_nonblockingintime_6hr * trackPoints_2D_mask
print('arr0/1 fake non-blocking with track preceding shape:', trackBlk_fake_non.shape, flush=True)
trackBlk_fake_non_num = np.nansum(trackBlk_fake_non,axis=0)
print('fake non-blocking with track preceding number at each lon:', trackBlk_fake_non_num, flush=True) # (360,), a value each day
track_B_freq_fake_non = (trackBlk_fake_non_num/trackdays) # frequency of fake non-blocking under track days
print('frequency of fake non-blocking under track days out of all track days:', track_B_freq_fake_non, flush=True)
...
#%% 07 make the final plot
fig, ax = plt.subplots(figsize=(10, 5))
ax.plot(Blklon, B_B_freq, color='red', label=f'High BAM, n={int(np.round(np.nanmean(B_B_day/4)))} days')
# ax.plot(Blklon, fake_B_B_freq, color='red', linestyle='--', label=f'FakeBlk High BAM, n={int(np.round(np.nanmean(B_B_day/4)))} days')

ax.plot(Blklon, track_B_freq, color='blue', label=f'with ACs preceding, n={int(np.round(np.nanmean(trackdays/4)))} days')
ax.plot(Blklon, track_B_freq_fake, color='gray', label=f'with ACs preceding (fakeBlocking), n={int(np.round(np.nanmean(trackdays/4)))} days')
ax.plot(Blklon, track_B_freq_fake_non, color='yellow', label=f'with ACs preceding (fakeinNonBlocking), n={int(np.round(np.nanmean(trackdays/4)))} days')
...
plt.tight_layout()
plt.show()
plt.savefig(f'withFake2Senario_BAMwithACs_BlockingFreq_AlongLon_{rgname}_Type{typeid}_{cyc}_{leadD}daylead.png', dpi=300)
