...
def generate_unique_random_lists(n, duration_distribution):
    """
...
for lon in range(blockingEidArr_6hr.shape[1]):
    ...
    shuffleIndex = generate_unique_random_lists(62824, BlockPersis)  # generate the random unique lists for each lon
    allIndex = [item for sublist in shuffleIndex for item in sublist]
    print(f'lon {lon} allIndex: {allIndex}', flush=True)
    fake_blockingEidArr[np.array(allIndex),lon] = 1
    
    print(f'lon {lon} done', flush=True)
    

np.save(f'/scratch/bell/hu1029/LGHW/fake_blockingEidArr_6hr_Type{typeid}_{rgname}_{ss}.npy', fake_blockingEidArr)
