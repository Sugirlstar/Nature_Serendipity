...
for i in range(np.shape(LWATargetArr)[0]):
    ...
    plt.show()
    plt.savefig(f'./AnimationMap/Blocking_LWA_EventID{testblkid}_Day_{i}.png', bbox_inches='tight', dpi=300)
    plt.close()
...
frames = sorted([
    ...
    if f.startswith(f'Blocking_LWA_EventID{testblkid}') and f.endswith('.png')
], key=lambda f: int(f.split('_')[-1].split('.')[0]))
print(frames)
# get all the images
gif_filename = f"./AnimationMap/Animation_EventID{testblkid}.gif"
