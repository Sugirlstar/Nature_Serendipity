    ...
    for typeid in [1]:
        for cyc in ["AC"]:
            for ssi,ss in enumerate(['ALL']):
                ...
                # intopercentList length = length of InteractingBlockID != -1 (not the full list)
                ...
                print(f'length of target block ids: {len(targetBlockID)}', flush=True)
                print(f'length of all interacting block ids: {len(InteractingBlockID_positive)}', flush=True)
                print(f'percent of target block ids (the <0.1 events): {len(targetBlockID)/len(InteractingBlockID_positive)}', flush=True)
                ...
                for jj in range(len(targetBlockID)):
                    ...
                    maskday[firstday-16:firstday] = True # extend the first day to 4 timesteps before
                    maskday[firstday+16:] = False # remove the days after the second day
                    ...
                    # get the target Z500 anomaly
                    ZanomTargetArr = Zanom[maskday, :, :] # get the Z500
                    ...
                    z500levels = np.linspace(np.nanmin(ZanomTargetArr), np.nanmax(ZanomTargetArr), 10)
                    pos_levels = [lev for lev in z500levels if lev > 0]
                    neg_levels = [lev for lev in z500levels if lev < 0]

                    print('cflevels:', cflevels, flush=True)
                    print('z500levels:', z500levels, flush=True)
                    ...
                    for col in range(2):              
                        for row in range(rows):     
                            ...
                            _, _, cf = create_Map(lonLWA, latLWA, LWATargetArr[i*4, :, :], fill=True, fig=fig, ax=ax,
                                                ...
                                                title=f'Blocking LWA on {timeBlock[i*4]}')
                            cf_last = cf
                            ax.set_title(f'LeadDay: {round(indicator/4,0)}', fontsize=10, loc='left')
                            # ax.add_feature(cfeature.COASTLINE.with_scale('110m'))
                            ax.gridlines(crs=ccrs.PlateCarree(), draw_labels=True, 
                                            linewidth=1, color='gray',linestyle='--',alpha=0.5)
                            # z500 anomaly contours (positive)
                            cf1 = ax.contour(
                                lonLWA, latLWA, ZanomTargetArr[i*4, :, :],
                                levels=pos_levels, colors='darkgreen', linewidths=1, linestyles='solid',
                                transform=ccrs.PlateCarree()
                            )
                            ax.clabel(cf1, inline=True, fontsize=8, fmt='%1.0f', colors='darkgreen', use_clabeltext=True)
                            # z500 anomaly contours (negative)
                            cf2 = ax.contour(
                                lonLWA, latLWA, ZanomTargetArr[i*4, :, :],
                                levels=neg_levels, colors='darkgreen', linewidths=1, linestyles='dashed',
                                transform=ccrs.PlateCarree()
                            )
                            # ax.clabel(cf2, inline=True, fontsize=8, fmt='%1.1f', colors='darkgreen', use_clabeltext=True)
                            ax.contour(lonLWA, latLWA, BlkTargetArr[i*4, :, :].astype(float), levels=[0.5], 
                            ...
                            for track_id, points in TargetperiodTrackList[i*4]:
                                addSegments(ax, points, colr='darkred', linewidth=2, alpha=0.4)
                                if len(points) > 0:
                                    end_lon, end_lat = points[-1]
                                    ax.plot(end_lon, end_lat, marker='x', color='darkred', markersize=7, 
                    ...
                    # 调整间距
                    plt.tight_layout()
                    plt.subplots_adjust(right=0.9)  # 数值 <1 缩小子图宽度，给右侧留空
                    cbar = fig.colorbar(cf_last, ax=axes, orientation='vertical', fraction=0.025, pad=0.02)
                    cbar.set_label('LWA')

                    # 保存整张 panel
                    plt.savefig(f'./new_results/BlkSeed_Type{typeid}_{cyc}_{rgname}_EventID{testblkid}_panels.png', 
