    ...
    for typeid in [1]:
        for cyc in ["AC"]:
            for ssi,ss in enumerate(['ALL']):
                ...
                InteractingBlockID_positive = np.where(InteractingBlockID != -1) # the index of the track that interact with blocking
                ...
                # ! entertime: length = length of InteractingBlockID = length of track_data
                entertime = np.load(f'/scratch/bell/hu1029/LGHW/EnterTimePointr2Blk1stDay_type{typeid}_{cyc}_{rgname}_{ss}.npy')
                print('CHECK1 entertime all:', entertime, flush=True)
                ...
                print(f'CHECK1.5 target entertime: {entertime[targetTrackID]}', flush=True)

                testblkid = 4
                print(f'CHECK2: testblkid {testblkid}', flush=True)
                ...
                print('CHECK 2.5 target Block event dates:', np.array(timei)[np.where(maskday0)[0]], flush=True)
                ...
                print('CHECK3 eddyindices:', eddyindices, flush=True)
                print('CHECK3 targetTracks:', targetTracks, flush=True)
                print('CHECK3.5 targetEntertime :', entertime[eddyindices], flush=True)
