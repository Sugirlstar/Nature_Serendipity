...
for i in range(len(datasets)):
    ...
    F128refF = F128refFile[dtname]
    Dg1refF = Dg1refFile[dtname]
    ...
    # read data --------------------------------------------------------------
    # F128 attributes
    ds = xr.open_dataset(F128refF)
    lonF128 = np.array(ds['lon'])
    latF128 = np.array(ds['lat']) # latitude increasing order (-90~90)
    ...
    print('F128 NH:', flush=True)
    print(latF128_NH) # 0.35087653~89.46282157
    print('F128 SH:', flush=True)
    print(lafF128_SH) # -90~-0.35087653

    # 1dg attributes
    ds1dg = xr.open_dataset(Dg1refF)
    ...
    print('1dg NH:', flush=True)
    print(lat1dgNH)
    print('1dg SH:', flush=True)
    print(lat1dgSH)
    ...
    def getTrackArr(trackType ,track_data, lonF128, latF128_NH, lon1dg, lat1dgNH, timeiarr):
        ...
        print(f"{trackType} track total number:", len(time_list), flush=True)
    ...
    for k in ['NH', 'SH']:
        for trackType in ['CC', 'AC']:

            with open(f'{OUT_DIR}/{dtname}_{trackType}Zanom_allyearTracks_{k}.pkl', 'rb') as file:
            ...
            np.save(f'{OUT_DIR}/{dtname}_{trackType}trackPoints_arrayF128_{k}.npy', trackPoints_array_k)
            np.save(f'{OUT_DIR}/{dtname}_{trackType}trackPoints_array1dg_{k}.npy', trackPoints_array_1dg_k)
            ...
            addSegments(ax, [(330,45),(30,45),(30,75),(330,75),(330,45)], colr='black', linewidth=2, linestyle='solid')
            plt.colorbar(cf,ax=ax,orientation='horizontal',label='Frequency (days)',fraction=0.04, pad=0.1)
            plt.show()
            plt.tight_layout()
            plt.savefig(f'{dtname}_{trackType}_pointFrequency_{k}_F128.png')
            plt.close()
            ...
            plt.show()
            plt.tight_layout()
            plt.savefig(f'{dtname}_{trackType}_pointFrequency_{k}_1dg.png')
            plt.close()
            
            # 02 save the track ID array - F128 and 1dg
            np.save(f'{OUT_DIR}/{dtname}_{trackType}trackPoints_TrackIDarray_{k}_F128.npy', trackPoints_ID_k)
            np.save(f'{OUT_DIR}/{dtname}_{trackType}trackPoints_TrackIDarray_{k}_1dg.npy', trackPoints_ID_1dg_k)

            # remove the arrays to save memory
            del trackPoints_array_k, trackPoints_ID_k, trackPoints_array_1dg_k, trackPoints_ID_1dg_k
